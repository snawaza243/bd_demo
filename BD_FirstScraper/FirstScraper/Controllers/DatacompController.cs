...
namespace FirstScraper.Controllers
{
    ...
    [ApiController]
    public class DatacompController : ControllerBase
    {
        private readonly ILogger<DatacompController> _logger;

        public DatacompController(ILogger<DatacompController> logger)
        {
            _logger = logger;
        }

        [HttpGet("scrape")]
        public async Task<IActionResult> ScrapeProduct([FromQuery] string url = "https://datacomp.sk/dell-poweredge-t350-rw9j2_d482121.html")
        {
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    //var productcategory = document.DocumentNode.SelectNodes("//*[@id=\"sticattree\"]//ul/li");
                    ...
                    var productcategory = document.DocumentNode.SelectNodes("//div[contains(@class,\'boxcont navbar\')]//ul/li/a");
                    if (productcategory != null)
                    {
                        product.productcategory = productcategory
                            .Select(row => row.InnerText.Trim())
                    ...
                    var nameNode = document.DocumentNode.SelectSingleNode("//div[contains(@class,\"hdr\")]/h1");
                    if (nameNode != null)
                    {
                    ...
                    //}
                    var imageNodes = document.DocumentNode.SelectNodes("//div[contains(@class,\"boxcont gallery\")]//img");
                    if (imageNodes != null)
                    {
                    ...
                    var specificationNodes = document.DocumentNode.SelectNodes("//div[contains(@id,\"stiparsets\")]//table[contains(@class,\"tabinfo hor\")]//tr");

                    if (specificationNodes != null)
                    {
                        foreach (var row in specificationNodes)
                        {
                            // Find key and value nodes with correct relative paths
                            var keyNode = row.SelectSingleNode(".//td[@class='paramname']/p");
                            var valueNode = row.SelectSingleNode(".//td[@class='paramvalue']/p");

                            if (keyNode != null && valueNode != null)
                            {
                                string specKey = keyNode.InnerText.Trim();
                                string specValue = valueNode.InnerText.Trim();
                                product.productspecification[specKey] = specValue;
                            }
                        }
                    }
                    ...
                    //var technicaldataNodes = document.DocumentNode.SelectNodes("//table[contains(@class,\'tabinfo hor\')]/tbody//tr");
