...
namespace FirstScraper.Controllers
{
    ...
    [ApiController]
    public class AustinController : ControllerBase
    {
        private readonly ILogger<AustinController> _logger;
        public AustinController(ILogger<AustinController> logger)
        {
            _logger = logger;
        }
        [HttpGet("scrape")]
        public async Task<IActionResult> ScrapeProduct([FromQuery] string url = "https://www.austin.net.au/products/msi-modern-15-b13m-407au-business-productivity-laptop-15-6-fhd-intel-raptor-lake-i5-1335u-onboard-ddr4-16gb-1tb-ssd-windows11-home-intel-iris-xe-graphics")
        {
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var productCategory = document.DocumentNode.SelectNodes("//div[contains(@class,\"page__sub-header\")]//ol/li");
                    ...
                    var nameNode = document.DocumentNode.SelectSingleNode("//h1[contains(@class,\"product-meta__title heading h1\")]");
                    if (nameNode != null)
                    {
                        product.Name = nameNode.InnerText.Trim();
                    }

                    var skuNode = document.DocumentNode.SelectSingleNode("//div[contains(@class,\"product-meta__reference\")]//span[1]/span");
                    if (skuNode != null)
                    {
                        product.sku = skuNode.InnerText.Trim();
                    }

                    var modelNode = document.DocumentNode.SelectSingleNode("//div[contains(@class,\"product-meta__reference\")]//span[2]/span");
                    if (modelNode != null)
                    {
                    ...
                    var mrp = document.DocumentNode.SelectSingleNode("//div[contains(@class,\"price-list\")]/span");
                    if (mrp != null)
                    {
                    ...
                    var imageNodes = document.DocumentNode.SelectNodes("//div[contains(@class,\"scroller\")]//a//div//img");
                    if (imageNodes != null)
                    {
                    ...
                    var descriptionRow = document.DocumentNode.SelectNodes("//div[contains(@class,\"card__section\")]//ul");
