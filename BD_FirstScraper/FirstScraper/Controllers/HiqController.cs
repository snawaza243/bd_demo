...
namespace FirstScraper.Controllers
{
    ...
    {
        private readonly ILogger<HiqController> _logger;
        public HiqController(ILogger<HiqController> logger)
        {
            _logger = logger;
        }
        [HttpGet("scrape")]
        public async Task<IActionResult> ScrapeProduct([FromQuery] string url = "https://hiq24.de/shop/hardware/festplatten-ssds/ssd-festplatte/m.2/93641/samsung-ssd-1tb-7.4/6.9g-990-pro-m.2-sam-nvme")
        {
            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    var productcategory = document.DocumentNode.SelectNodes("//nav[contains(@class,\"content--breadcrumb block\")]//li//a/span");
                    ...
                    var nameNode = document.DocumentNode.SelectSingleNode("//div[contains(@class,\"product--buybox block\")]//h1");
                    if (nameNode != null)
                    {
                    ...
                    var skuNode = document.DocumentNode.SelectSingleNode("//ul[contains(@class,\"product--base-info list--unstyled\")]//li[1]//span");
                    if (skuNode != null)
                    {
                        product.sku = skuNode.InnerText.Trim();
                    }

                    var modelNode = document.DocumentNode.SelectSingleNode("//ul[contains(@class,\"product--base-info list--unstyled\")]//li[2]//span");
                    if (modelNode != null)
                    {
                        product.Model = modelNode.InnerText.Trim();
                    }

                    var mrp = document.DocumentNode.SelectSingleNode("//div[contains(@class,\"product--price price--default\")]/span");
                    if (mrp != null)
                    {
                    ...
                    //}
                    var imageNodes = document.DocumentNode.SelectNodes("//div[contains(@class, 'image-slider--container')]//span[@class='image--element']//img | //div[contains(@class, 'image-slider--thumbnails-slide')]//img");

                    if (imageNodes != null)
                    {
                        product.ImageUrl = imageNodes
                            .Select(node => node.GetAttributeValue("src", string.Empty) ?? node.GetAttributeValue("srcset", string.Empty))
